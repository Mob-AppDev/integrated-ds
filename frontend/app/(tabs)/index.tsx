...
export default function HomeScreen() {
  ...
  const loadChannels = async () => {
    try {
      ...
      setChannels(userChannels.map(channel => ({
  ...
  // Debug: log filter state and filteredDMs
  console.log('sortBy:', sortBy, 'showType:', showType, 'unreadPriority:', unreadPriority, 'filteredDMs:', filteredDMs);
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.card }]}>
        <View style={styles.workspaceInfo}>
          <View style={[styles.workspaceIcon, { backgroundColor: theme.accent }]}> 
            <Text style={[styles.workspaceIconText, { color: theme.text }]}>
              {user?.username?.substring(0, 2).toUpperCase() || 'DS'}
            </Text>
          </View>
          <Text style={[styles.workspaceName, { color: theme.text }]}>
            {user?.username ? `${user.username}'s Workspace` : 'DevSync Workspace'}
          </Text>
        </View>
        <TouchableOpacity 
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.quickActions}>
          ...
          <TouchableOpacity style={styles.quickAction} onPress={() => router.push('/(tabs)/huddles-menu')}>
            <Coffee size={20} color="#FFFFFF" />
          ...
          {channelsExpanded && (
            isLoading ? (
              <View style={{ paddingHorizontal: 16, paddingVertical: 8 }}>
                <Text style={{ color: theme.tabInactive }}>Loading channels...</Text>
              </View>
            ) : (
