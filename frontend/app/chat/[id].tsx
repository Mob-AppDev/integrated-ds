...
import { messageService, MessageWithUI } from '../../services/messageService';
...
export default function ChatScreen() {
  ...
  const loadMessages = async () => {
    try {
      setIsLoading(true);
      let loadedMessages: MessageWithUI[] = [];
      
      if (typeof id === 'string' && id.startsWith('dm-')) {
        const userId = parseInt(id.replace('dm-', ''), 10);
        loadedMessages = await messageService.getDirectMessages(userId);
      } else if (typeof id === 'string') {
        const channelId = parseInt(id, 10);
        loadedMessages = await messageService.getChannelMessages(channelId);
      }
    ...
    try {
      ...
      if (res && !res.canceled && res.assets && res.assets.length > 0) {
        ...
        if (uri) {
          ...
          Alert.alert('File Upload', 'File upload feature coming soon!');
          setTimeout(() => {
  ...
  const sendMessage = async () => {
    ...
    try {
      setMessageText('');
      let newMessage: MessageWithUI;
      
      if (typeof id === 'string' && id.startsWith('dm-')) {
        const userId = parseInt(id.replace('dm-', ''), 10);
        newMessage = await messageService.sendDirectMessage(userId, content);
      } else if (typeof id === 'string') {
        const channelId = parseInt(id, 10);
        newMessage = await messageService.sendChannelMessage(channelId, content);
      } else {
  ...
  const handleMessageLongPress = (message: MessageWithUI) => {
  ...
  const renderMessage = (message: MessageWithUI) => (
    <TouchableOpacity
    ...
    >
      <View style={styles.messageHeader}>
        ...
        <Text style={[styles.senderName, { color: theme.text }]}>{message.sender.username}</Text>
      ...
      <View style={styles.messageContent}>
        ...
        <Text style={[styles.messageText, { color: theme.text }]}>{message.content}</Text>
      </View>
        ...
        <View style={styles.reactionsContainer}>
          {message.reactions.map((reaction, index) => (
            <TouchableOpacity
              ...
              onPress={() => handleReaction(message.id.toString(), reaction.emoji)}
            >
