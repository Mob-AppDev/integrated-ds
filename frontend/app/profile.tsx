    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await updateUserStatus('ACTIVE', true);
              setAwayStatus(false);
            } catch (error) {
        ...
        { 
          ...
          onPress: async () => {
            try {
              await updateUserStatus('AWAY', false);
              setAwayStatus(true);
            } catch (error) {
              console.error('Status update failed:', error);
            }
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.profileSection, { borderBottomWidth: 0 }]}> 
          ...
          <View style={styles.profileInfo}>
            <Text style={[styles.profileName, { color: theme.text }]}>{user?.username || 'User'}</Text>
            <View style={styles.statusContainer}>
              <View style={[styles.statusIndicator, { backgroundColor: user?.isOnline ? '#4CAF50' : '#FFA726' }]} />
        ...
        <View style={[styles.section, { borderBottomWidth: 0 }]}> 
          ...
          <TouchableOpacity style={styles.menuItem} onPress={handleStatusChange}>
            ...
            <Switch
              value={user?.status === 'AWAY'}
              onValueChange={async (value) => {
                try {
                  await updateUserStatus(value ? 'AWAY' : 'ACTIVE', !value);
                } catch (error) {
                  console.error('Status update failed:', error);
                }
              }}
        ...
        <View style={[styles.section, { borderBottomWidth: 0 }]}> 
          ...
          <TouchableOpacity style={styles.menuItem}>
            ...
            <Text style={[styles.menuText, { color: theme.text }]}>{user?.email || 'No email'}</Text>
          </TouchableOpacity>
