@echo off
REM DevSync Database Setup Script for Windows
...
"C:\Program Files\PostgreSQL\17\bin\psql.exe" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -f schema.sql

echo ✅ Schema created successfully

REM Insert sample data
echo 📊 Inserting sample data...
"C:\Program Files\PostgreSQL\17\bin\psql.exe" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -f sample_data.sql

echo ✅ Sample data inserted

REM Verify setup
echo 🔍 Verifying database setup...
for /f %%i in ('"C:\Program Files\PostgreSQL\17\bin\psql.exe" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -t -c "SELECT COUNT(*) FROM users;"') do set USER_COUNT=%%i
for /f %%i in ('"C:\Program Files\PostgreSQL\17\bin\psql.exe" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -t -c "SELECT COUNT(*) FROM channels;"') do set CHANNEL_COUNT=%%i
for /f %%i in ('"C:\Program Files\PostgreSQL\17\bin\psql.exe" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -t -c "SELECT COUNT(*) FROM messages;"') do set MESSAGE_COUNT=%%i
